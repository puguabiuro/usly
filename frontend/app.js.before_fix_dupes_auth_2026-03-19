...
const App = {
  ...
  // Sub-states / filters
  eventsTab: "for_you", // 'nearby' | 'followed'
  ...
  // Demo data
  people: [
    { id: "3", nick: "Maja", city: "Warszawa", age: 26, interests: ["kawa", "joga", "muzyka"], emoji: "🌸" },
    { id: "3", nick: "Alex", city: "Warszawa", age: 29, interests: ["kino", "gry", "AI"], emoji: "🎧" },
    { id: "3", nick: "Kasia", city: "Warszawa", age: 24, interests: ["spacer", "psy", "fotografia"], emoji: "📸" },
    { id: "3", nick: "Tomek", city: "Warszawa", age: 31, interests: ["rower", "góry", "kawa"], emoji: "🚴" },
  ],
...
if (App.role === "user") {
  await Promise.all([loadNearbyPeople(), loadEvents(), loadMyGroups(), loadGroups()]);
}
...
async function setUserPlan(plan) {
  ...
  if (App.isLoggedIn && App.role === "user") {
    try {
      ...
      refreshUserPlanCardsUi();
      renderAll();
      toast(`Wybrano plan: ${App.user.plan.toUpperCase()}`);
      return;
    } catch (err) {
...
function setPersonChatButtonState(state) {
  ...
  if (state === "pending") {
    btn.textContent = "Czat po akceptacji";
    btn.disabled = true;
    btn.style.opacity = "0.7";
    return;
  }

  btn.textContent = "Dodaj najpierw do znajomych";
  btn.disabled = true;
  btn.style.opacity = "0.7";
}
...
async function addFriendFromProfile() {
  ...
  const token = localStorage.getItem(USLY_STORAGE_KEYS.token);
  if (!token) {
  ...
  try {
    ...
    if (!res.ok) {
      toast(data.detail || "Nie udało się wysłać zaproszenia");
      return;
    }
    ...
    toast("Błąd połączenia przy wysyłaniu zaproszenia");
  }
}
...
async function refreshProfileRelations() {
  ...
  try {
    ...
    const friends = Array.isArray(friendsPayload) ? friendsPayload : [];
...
function getChatOtherPerson() {
  ...
  return App.people.find(p => String(p.id) === String(App.selectedChatUserId)) || null;
}
...
function openChatParticipantProfile(userId) {
  ...
  const person = App.people.find(p => String(p.id) === String(userId));
  if (!person) {
...
function renderEventsList() {
  ...
  list.innerHTML = events.map(ev => `
    <div class="listItem" onclick="openEvent('${ev.id}')">
      ...
      <div class="listBody">#${ev.interest} • ${getEventCapacityCopy(ev).line}${getEventCapacityCopy(ev).alert ? ` • ${getEventCapacityCopy(ev).alert}` : ""}</div>
    </div>
  `).join("");
}
...
async function init() {
  ...
  Promise.all([loadNearbyPeople(), loadEvents(), loadMyGroups(), loadGroups()]).finally(() => {
    renderAll();
    bindMessageInputs();
  });
}
...
function renderNearbyMapMarkers() {
  ...
  App.people.slice(0, 12).forEach((person, index) => {
    ...
    const marker = L.marker([lat, lng], { icon: personIcon })
      .addTo(nearbyMap)
      .bindPopup("<b>" + person.nick + "</b><br>" + (person.city || "W pobliżu"));

    marker.on("click", () => openMapMarker("person", index));
    nearbyMarkers.push(marker);
  });

  App.events.slice(0, 12).forEach((ev, index) => {
