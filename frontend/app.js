...
const I18N = {
  pl: {
    ...
    "partnerCreate.namePlaceholder": "np. Koncert na żywo",
    ...
    "partnerCreate.addressPlaceholder": "np. Padma Joga Kobyłka albo ul. Nadmeńska 5C",
  ...
  en: {
    ...
    "partnerCreate.addressPlaceholder": "e.g. Padma Yoga Kobyłka or Nadmeńska 5C",
...
function go(viewId) {
  ...
  if (viewId === "S_ADMIN_REPORTS" && typeof reloadAdminReports === "function") {
...
function updateTabbars() {
  ...
  if (!App.isLoggedIn || publicViews.includes(App.currentView) || App.role === "admin" || App.currentView === "S_ADMIN_REPORTS") {
    hide(userBar);
    hide(partnerBar);
    return;
  }
...
async function reloadAdminReports() {
  const userBox = document.getElementById("adminUserReportsList");
  const eventBox = document.getElementById("adminEventReportsList");
  const bugBox = document.getElementById("adminBugReportsList");

  if (userBox) userBox.innerHTML = `<div class="emptyState">Ładowanie...</div>`;
  if (eventBox) eventBox.innerHTML = `<div class="emptyState">Ładowanie...</div>`;
  if (bugBox) bugBox.innerHTML = `<div class="emptyState">Ładowanie...</div>`;

  try {
    const [userRes, eventRes, bugRes] = await Promise.all([
      window.apiFetch("/admin/user-reports"),
      window.apiFetch("/admin/event-reports"),
      fetch(`${API_BASE_URL}/admin/bug-reports`).then(r => r.json()),
    ]);

    const selectedStatus = document.getElementById("adminReportsStatusFilter")?.value || "all";
    const filterItems = (items) => selectedStatus === "all"
      ? items
      : items.filter((r) => String(r.status || "new") === selectedStatus);

    renderAdminUserReports(filterItems(Array.isArray(userRes?.data) ? userRes.data : []));
    renderAdminEventReports(filterItems(Array.isArray(eventRes?.data) ? eventRes.data : []));
    renderAdminBugReports(filterItems(Array.isArray(bugRes?.data) ? bugRes.data : []));
  } catch (e) {
    console.error("reloadAdminReports error", e);
    if (userBox) userBox.innerHTML = `<div class="emptyState">Nie udało się pobrać zgłoszeń użytkowników.</div>`;
    if (eventBox) eventBox.innerHTML = `<div class="emptyState">Nie udało się pobrać zgłoszeń wydarzeń.</div>`;
    if (bugBox) bugBox.innerHTML = `<div class="emptyState">Nie udało się pobrać zgłoszeń błędów.</div>`;
  }
}
...
function adminStatusPill(status) {
  return `<span class="pill">${escapeAdminHtml(adminStatusLabel(status))}</span>`;
}

function adminStatusQuickActions(reportType, ticket, currentStatus) {
  const current = String(currentStatus || "new");
  const opts = reportType === "bug"
    ? [["accepted","Przyjęte"],["in_progress","W trakcie"],["fixed","Naprawione"],["not_reproducible","Nie do odtworzenia"],["archived","Archiwum"]]
    : [["in_review","Do obserwacji"],["resolved","Rozwiązane"],["rejected","Odrzucone"],["archived","Archiwum"]];

  return `
    <div style="display:flex;flex-wrap:wrap;gap:7px;margin-top:10px;">
      ${opts.map(([value,label]) => {
        const active = value === current;
        const bg = active ? "rgba(34,211,238,.20)" : "rgba(255,255,255,.08)";
        const opacity = active ? ".55" : "1";
        const disabled = active ? "disabled" : "";
        const click = active ? "" : `onclick="adminChangeReportStatus('${escapeAdminHtml(reportType)}','${escapeAdminHtml(ticket)}','${value}')"`;
        return `<button type="button" ${click} ${disabled} style="border:1px solid ${active ? "rgba(34,211,238,.38)" : "rgba(255,255,255,.18)"};background:${bg};color:#f5f7ff;border-radius:999px;padding:7px 10px;font-size:12px;font-weight:900;cursor:${active ? "not-allowed" : "pointer"};opacity:${opacity};">${escapeAdminHtml(label)}</button>`;
      }).join("")}
    </div>
  `;
}

function adminNextAction(reportType, ticket, status) {
  const st = String(status || "new");

  if (reportType === "bug") {
    if (st === "new") return adminMiniAction("Przyjmij", `adminChangeReportStatus('bug','${escapeAdminHtml(ticket)}','accepted')`);
    if (st === "accepted") return adminMiniAction("W trakcie", `adminChangeReportStatus('bug','${escapeAdminHtml(ticket)}','in_progress')`);
    if (st === "in_progress") return adminMiniAction("Naprawione", `adminChangeReportStatus('bug','${escapeAdminHtml(ticket)}','fixed')`, "success");
    return adminMiniAction("Zamknięte", "", "secondary", true);
  }

  if (st === "new") return adminMiniAction("Do obserwacji", `adminChangeReportStatus('${escapeAdminHtml(reportType)}','${escapeAdminHtml(ticket)}','in_review')`);
  if (st === "in_review") return adminMiniAction("Rozwiąż", `adminChangeReportStatus('${escapeAdminHtml(reportType)}','${escapeAdminHtml(ticket)}','resolved')`, "success");
  return adminMiniAction("Zamknięte", "", "secondary", true);
}


function adminStatusSelect(reportType, ticket, status) {
  ...
  const opts = reportType === "bug" ? bugOpts : moderationOpts;

  return `
    <select class="mt12" onchange="adminChangeReportStatus('${escapeAdminHtml(reportType)}','${escapeAdminHtml(ticket)}',this.value)">
      ${opts.map(([value, label]) => `<option value="${value}" ${value === current ? "selected" : ""}>${label}</option>`).join("")}
    </select>
  `;
}

async function adminChangeReportStatus(reportType, ticket, status) {
  try {
...
window.adminChangeReportStatus = adminChangeReportStatus;

function adminMiniAction(label, onclick, variant = "secondary", disabled = false) {
  const bg = disabled
    ? "rgba(255,255,255,.04)"
    : variant === "danger"
      ? "rgba(239,68,68,.18)"
  ...
  return `<button type="button" ${click} ${disabled ? "disabled" : ""} style="border:1px solid rgba(255,255,255,.18);background:${bg};color:#f5f7ff;border-radius:999px;padding:5px 8px;font-size:11px;font-weight:850;line-height:1;cursor:${cursor};white-space:nowrap;opacity:${opacity};">${escapeAdminHtml(label)}</button>`;
}


function renderAdminUserReports(items) {
  const box = document.getElementById("adminUserReportsList");
  if (!box) return;
  if (!items.length) {
    box.innerHTML = `<div class="emptyState">Brak zgłoszeń użytkowników.</div>`;
    return;
  }

  box.innerHTML = items.map((r) => {
    const ticket = escapeAdminHtml(r.ticket || "UR");
    const status = String(r.status || "new");
    return `
      <div class="card mt6" style="padding:7px 9px;border-radius:13px;">
        <div style="display:grid;grid-template-columns:minmax(72px,.8fr) minmax(0,1.7fr) auto;gap:8px;align-items:center;">
          <div>
            <strong style="font-size:13px;">${ticket}</strong>
            <div class="sectionSub" style="font-size:10.5px;line-height:1.1;">${escapeAdminHtml(r.created_at || "—")}</div>
          </div>
          <div style="min-width:0;">
            <div style="display:flex;gap:6px;align-items:center;min-width:0;">
              ${adminStatusPill(status)}
              <span class="sectionSub" style="font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeAdminHtml(r.reason_label || r.reason || "Bez powodu")}</span>
            </div>
            <div class="sectionSub mt4" style="font-size:11.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">user #${escapeAdminHtml(r.reported_user_id)} · zgł. #${escapeAdminHtml(r.reporter_user_id)}</div>
          </div>
          <div style="display:flex;gap:5px;align-items:center;justify-content:flex-end;">
            ${adminMiniAction("Podgląd", `openAdminUserPreview('${escapeAdminHtml(r.reported_user_id)}','${ticket}','${escapeAdminHtml(status)}')`)}
            ${adminNextAction("user", ticket, status)}
          </div>
        </div>
      </div>
    `;
...
function renderAdminEventReports(items) {
  const box = document.getElementById("adminEventReportsList");
  if (!box) return;
  if (!items.length) {
    box.innerHTML = `<div class="emptyState">Brak zgłoszeń wydarzeń.</div>`;
    return;
  }

  box.innerHTML = items.map((r) => {
    const ticket = escapeAdminHtml(r.ticket || "ER");
    const status = String(r.status || "new");
    return `
      <div class="card mt6" style="padding:7px 9px;border-radius:13px;">
        <div style="display:grid;grid-template-columns:minmax(72px,.8fr) minmax(0,1.7fr) auto;gap:8px;align-items:center;">
          ...
          <div style="min-width:0;">
            <div style="display:flex;gap:6px;align-items:center;min-width:0;">
              ${adminStatusPill(status)}
              <span class="sectionSub" style="font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeAdminHtml(r.event_title || "Wydarzenie")}</span>
            </div>
            <div class="sectionSub mt4" style="font-size:11.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">event #${escapeAdminHtml(r.event_id)} · ${escapeAdminHtml(r.reason_label || r.reason || "—")}</div>
          </div>
          <div style="display:flex;gap:5px;align-items:center;justify-content:flex-end;">
            ${adminMiniAction("Podgląd", `openAdminEventPreview('${escapeAdminHtml(r.event_id)}','${ticket}','${escapeAdminHtml(status)}')`)}
            ${adminNextAction("event", ticket, status)}
          </div>
        </div>
      </div>
    `;
...
function renderAdminBugReports(items) {
  const box = document.getElementById("adminBugReportsList");
  if (!box) return;
  if (!items.length) {
    box.innerHTML = `<div class="emptyState">Brak zgłoszeń błędów.</div>`;
    return;
  }

  box.innerHTML = items.map((r) => {
    const ticket = escapeAdminHtml(r.ticket || r.id || "");
    const status = String(r.status || "new");
    return `
      <div class="card mt6" style="padding:7px 9px;border-radius:13px;">
        <div style="display:grid;grid-template-columns:minmax(82px,.85fr) minmax(0,1.8fr) auto;gap:8px;align-items:center;">
          <div>
            <strong style="font-size:13px;">Bug #${ticket || "—"}</strong>
            <div class="sectionSub" style="font-size:10.5px;line-height:1.1;">${escapeAdminHtml(r.created_at || r.createdAt || r.date || "—")}</div>
          </div>
          <div style="min-width:0;">
            <div style="display:flex;gap:6px;align-items:center;min-width:0;">
              ${adminStatusPill(status)}
              <span class="sectionSub" style="font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeAdminHtml(r.current_view || "—")}</span>
            </div>
            <div class="sectionSub mt4" style="font-size:11.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeAdminHtml(r.role || "—")} · ${escapeAdminHtml(r.message || r.description || r.body || "Brak opisu.")}</div>
          </div>
          <div style="display:flex;gap:5px;align-items:center;justify-content:flex-end;">
            ${adminMiniAction("Podgląd", `openAdminBugPreview('${ticket}')`)}
            ${adminNextAction("bug", ticket, status)}
          </div>
        </div>
      </div>
    `;
...
async function openAdminBugPreview(ticket) {
  try {
    const res = await fetch(`${API_BASE_URL}/admin/bug-reports`).then(r => r.json());
    ...
    openModal("Podgląd błędu", `
      <div data-hide-modal-footer="1" style="display:none;"></div>
      <div class="card">
        <div class="sectionTitle">Bug #${escapeAdminHtml(bug.ticket || bug.id || "—")}</div>
        <div class="sectionSub mt8">Status: ${escapeAdminHtml(adminStatusLabel(bug.status || "new"))}</div>
        <div class="sectionSub mt8">Data: ${escapeAdminHtml(bug.created_at || bug.createdAt || bug.date || "—")}</div>
        <div class="sectionSub mt8">Rola: ${escapeAdminHtml(bug.role || "—")} · Widok: ${escapeAdminHtml(bug.current_view || "—")}</div>
        <div class="sectionSub mt8">Kontakt: ${escapeAdminHtml(bug.email || bug.contact || "—")}</div>
        <div class="mt16">${escapeAdminHtml(bug.message || bug.description || bug.body || "Brak opisu.")}</div>
        <hr class="mt16" />
        <div class="sectionSub mt16">Obsługa zgłoszenia</div>
        ${adminStatusQuickActions("bug", bug.ticket || bug.id || "", bug.status || "new")}
...
async function adminQuickResolveReport(reportType, ticket) {
  await adminChangeReportStatus(reportType, ticket, "resolved");
}
window.adminQuickResolveReport = adminQuickResolveReport;

async function openAdminUserPreview(userId, ticket, reportStatus = "new") {
  try {
    const res = await window.apiFetch(`/admin/users/${encodeURIComponent(userId)}/preview`);
    const u = res?.data || {};
    openModal("Podgląd profilu", `
      <div data-hide-modal-footer="1" style="display:none;"></div>
      <div class="card">
        <div class="sectionTitle">${escapeAdminHtml(u.nick || "Brak nicku")}</div>
        <div class="sectionSub mt8">ID: ${escapeAdminHtml(u.id)} · ${escapeAdminHtml(u.email || "—")}</div>
        <div class="row mt16" style="gap:10px;align-items:stretch;">
          <div class="card" style="flex:1;min-width:0;margin:0;padding:12px;border-radius:18px;"><div class="sectionSub">Zgłoszenia</div><div class="sectionTitle">${escapeAdminHtml(u.reports_total || 0)}</div></div>
          <div class="card" style="flex:1;min-width:0;margin:0;padding:12px;border-radius:18px;"><div class="sectionSub">Otwarte</div><div class="sectionTitle">${escapeAdminHtml(u.reports_open || 0)}</div></div>
          <div class="card" style="flex:1;min-width:0;margin:0;padding:12px;border-radius:18px;"><div class="sectionSub">Konto</div><div class="sectionTitle" style="font-size:18px;overflow-wrap:anywhere;">${escapeAdminHtml(u.status || "—")}</div></div>
        </div>
        <div class="sectionSub mt16">Miasto: ${escapeAdminHtml(u.city || "—")} · Rola: ${escapeAdminHtml(u.role || "—")}</div>
        <div class="mt12">${escapeAdminHtml(u.bio || "Brak bio.")}</div>
        <hr class="mt16" />
        <div class="sectionSub mt16">Status zgłoszenia</div>
        ${adminStatusQuickActions("user", ticket, reportStatus)}
        <div class="sectionSub mt16">Akcja na koncie</div>
        <button class="btn mt12" type="button" onclick="adminSetUserStatus('${escapeAdminHtml(u.id)}','${String(u.status) === "blocked" ? "active" : "blocked"}')">${String(u.status) === "blocked" ? "Odblokuj konto" : "Zablokuj konto"}</button>
        <button class="btn secondary mt12" type="button" onclick="closeModal()">Zamknij</button>
      </div>
    `);
  } catch (e) {
    console.error("openAdminUserPreview error", e);
    toast(e?.userMessage || "Nie udało się otworzyć podglądu profilu");
  }
}
window.openAdminUserPreview = openAdminUserPreview;

async function openAdminEventPreview(eventId, ticket, reportStatus = "new") {
  try {
    ...
    openModal("Podgląd wydarzenia", `
      ...
      <div class="card">
        ...
        <div class="row mt16" style="gap:10px;align-items:stretch;">
          ...
          <div class="card" style="flex:1;min-width:0;margin:0;padding:12px;border-radius:18px;"><div class="sectionSub">Otwarte</div><div class="sectionTitle">${escapeAdminHtml(ev.reports_open || 0)}</div></div>
          <div class="card" style="flex:1;min-width:0;margin:0;padding:12px;border-radius:18px;"><div class="sectionSub">Event</div><div class="sectionTitle" style="font-size:18px;overflow-wrap:anywhere;">${escapeAdminHtml(ev.status || "—")}</div></div>
        </div>
        <div class="sectionSub mt16">Organizator: #${escapeAdminHtml(ev.partner_user_id || "—")}</div>
        <div class="sectionSub mt8">Miasto: ${escapeAdminHtml(ev.city || "—")} · Miejsce: ${escapeAdminHtml(ev.where || "—")}</div>
        <div class="sectionSub mt8">Zapisani: ${escapeAdminHtml(ev.signups_count || 0)} · Obserwujący: ${escapeAdminHtml(ev.saves_count || 0)}</div>
        <div class="mt12">${escapeAdminHtml(ev.description || "Brak opisu.")}</div>
        <hr class="mt16" />
        <div class="sectionSub mt16">Status zgłoszenia</div>
        ${adminStatusQuickActions("event", ticket, reportStatus)}
        <div class="sectionSub mt16">Akcja na wydarzeniu</div>
        <button class="btn mt12" type="button" onclick="adminSetEventStatus('${escapeAdminHtml(ev.id)}','${String(ev.status) === "archived" ? "published" : "archived"}')">${String(ev.status) === "archived" ? "Przywróć wydarzenie" : "Zarchiwizuj wydarzenie"}</button>
        <button class="btn secondary mt12" type="button" onclick="closeModal()">Zamknij</button>
      </div>
    `);
  } catch (e) {
    console.error("openAdminEventPreview error", e);
    toast(e?.userMessage || "Nie udało się otworzyć podglądu wydarzenia");
  }
}
window.openAdminEventPreview = openAdminEventPreview;

async function adminSetUserStatus(userId, status) {
  try {
    await window.apiFetch(`/admin/users/${encodeURIComponent(userId)}/status`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    });
    toast(status === "blocked" ? "Konto zablokowane" : "Konto odblokowane");
    closeModal();
    await reloadAdminReports();
  } catch (e) {
    console.error("adminSetUserStatus error", e);
    toast(e?.userMessage || "Nie udało się zmienić statusu konta");
  }
}
...
async function adminSetEventStatus(eventId, status) {
  try {
    ...
    toast(status === "archived" ? "Wydarzenie zarchiwizowane" : "Wydarzenie przywrócone");
    closeModal();
    await reloadAdminReports();
  } catch (e) {
    console.error("adminSetEventStatus error", e);
    toast(e?.userMessage || "Nie udało się zmienić statusu wydarzenia");
  }
}
